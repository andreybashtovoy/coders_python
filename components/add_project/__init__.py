...
class AddProject(Menu):
    IN_PAGE = 4

    def __init__(self, updater: Updater):
        super().__init__(updater, 'components/add_project/add_project.xml', 'addp')
    ...
    def initial_state(self, update: Update):
        ...
        if len(temp) > 1 and temp[1].lstrip('-').isdigit():
            dur = temp[1]
    ...
    def text_format(self, message_text, update: Update, state):
        ...
        if state['a'] != '0':
            name = DB.get_project_by_id(int(state['a']))['name']
    ...
    def get_activity_buttons(self, state, update: Update):
        ...
        activity_names = DB.get_user_projects(state['u_id'])
    ...
    def select_activity(self, state, update: Update, name):
        activity = DB.get_user_project_by_name(name, state['u_id'])
        state['a'] = activity['id']
        return state
    ...
    def done(self, update: Update, state):
        ...
        if state['a'] == '0':
            update.callback_query.answer(text="Ты не выбрал проект.", show_alert=True)
            return False
        ...
        project = DB.get_project_by_id(state['a'])

        activity = DB.get_activity_by_project_id(project['id'])

        if project is not None:
            DB.add_activity(state['u_id'], activity['id'], int(state['dur'])/60, project['id'])
        else:
